...
Using the `CryptoSession` constructor allow you to have access to a DRM
session.
...
:param cipherAlgorithm: string - Algorithm used for encryption / decryption
ciphers
:param macAlgorithm: string - Algorithm used for sign / verify
:raises RuntimeException: If the session can not be established
 @python_v18 New class added.Example::

    ..
    uuid_widevine = 'edef8ba9-79d6-4ace-a3c8-27dcd51d21ed'
    crypto_session = xbmcdrm.CryptoSession(uuid_widevine, 'AES/CBC/NoPadding',
'HmacSHA256')
    ..
"""
...
class CryptoSession:
    ...
    **Kodi's DRM class.**
    ...
    def GetKeyRequest(self, init: Union[str, bytearray],
        ...
        """
        \python_func{ GetKeyRequest(init, mimeType, offlineKey, optionalParameters) }
        ...
        :return: byte - The opaque key request data (challenge) which is send to key server
         ...
         @python_v18 New function added. @python_v19 With python 3 the init
        param must be a bytearray instead of byte.
        """
        return bytearray()
    
    def GetPropertyString(self, name: str) -> str:
        """
        \python_func{ GetPropertyString(name) } Request a system specific property value
        of the DRM system.
    ...
    def ProvideKeyResponse(self, response: Union[str, bytearray]) -> str:
        """
        \python_func{ ProvideKeyResponse(response) } Provide a key response

         When a key response is received from the license server, must be sent to the
        DRM instance by using provideKeyResponse. See also GetKeyRequest.

        :param response: byte - Key data returned from the license server
        :return: A keySetId if the response is for an offline key requests which can be used later with restoreKeys, else return empty for streaming key requests.

         @python_v18 New function added. @python_v19 With python 3 the
        response argument must be a bytearray instead of byte.
        """
        return ""
    
    def RemoveKeys(self) -> None:
        """
        \python_func{`RemoveKeys()` } Removes all keys currently loaded in a session.
    ...
    def RestoreKeys(self, keySetId: str) -> None:
        """
        \python_func{ RestoreKeys(keySetId) } Restores session keys stored during
        previous `ProvideKeyResponse` call.
    ...
    def SetPropertyString(self, name: str, value: str) -> None:
        """
        \python_func{ SetPropertyString(name, value) } Set a system specific property
        value in the DRM system.
    ...
    def Decrypt(self, cipherKeyId: Union[str, bytearray],
        ...
        """
        \python_func{ Decrypt(cipherKeyId, input, iv) } Decrypt an encrypted data by
        using session keys.
        ...
        :return: Decrypted input data
         ...
         @python_v18 New function added. @python_v19 With python 3 all
        arguments need to be of type bytearray instead of byte.
        """
        return bytearray()
    ...
    def Encrypt(self, cipherKeyId: Union[str, bytearray],
                ...
                iv: Union[str, bytearray]) -> bytearray:
        """
        \python_func{ Encrypt(cipherKeyId, input, iv) } Encrypt data by using session
        keys.
    ...
    def Sign(self, macKeyId: Union[str, bytearray],
        ...
        """
        \python_func{ Sign(macKeyId, message) } Generate a DRM encrypted signature for a
        text message.
    ...
    def Verify(self, macKeyId: Union[str, bytearray],
        ...
        """
        \python_func{ Verify(macKeyId, message, signature) } Verify the validity of a
        DRM signature of a text message.
        ...
        :return: true when the signature is valid
         ...
         @python_v18 New function added. @python_v19 With python 3 for all
        arguments is needed to pass bytearray instead of byte.
        """
        return True
